...
import sys
sys.path.append('/mnt/workspace/AgileFormer/contrast_models/CASCADE_lib')
...
class PVT_CASCADE(nn.Module):
    def __init__(self, num_classes=1,ds=False):
        ...
        path = '/mnt/workspace/AgileFormer/pvt_v2_b2.pth'
...
class TransCASCADE(nn.Module):
    ...
    zero_head=False, vis=False,ds=False):
...
def Rolling_Unet_S(num_classes=9,input_channels=3,
                         deep_supervision=False,img_size=512,
                         vit_patches_size=16,config_vit=get_r50_b16_config(),):
    config_vit.patches.grid = (int(img_size / vit_patches_size),
        int(img_size / vit_patches_size))
    print(f'config_vit.patches.grid:{config_vit.patches.grid}')
    model=TransCASCADE(config=config_vit,num_classes=num_classes,ds=deep_supervision)
    model.load_from(weights=np.load(config_vit.pretrained_path))
    return model


if __name__ == '__main__':
    model =Rolling_Unet_S().cuda()
