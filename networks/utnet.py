...
class Rolling_Unet_S(nn.Module):
    ...
    def __init__(self, num_classes, input_channels,deep_supervision,img_size,
    base_chan=32,  
    reduce_size=8, block_list='234', num_blocks=[1, 2, 4], projection='interp', num_heads=[2,4,8], attn_drop=0., proj_drop=0., bottleneck=False, maxpool=True, rel_pos=True, aux_loss=False):
        super().__init__()

        self.aux_loss = aux_loss
        self.inc = [BasicBlock(input_channels, base_chan)]
        if '0' in block_list:
...
if __name__=='__main__':
    ...
    model=Rolling_Unet_S(1,3,False,256,32)
    out=model(input)
