...
# num_classes, input_channels=3, 
#             deep_supervision=False,
#             img_size=224,
class Rolling_Unet_S(nn.Module):
    def __init__(self, config=get_hiformer_b_configs(),
                 num_classes=9,input_channels=3,deep_supervision=False,
                  img_size=224,):
        super().__init__()
        self.img_size = img_size
        self.patch_size = [4, 16]
        self.n_classes = num_classes
        self.All2Cross = All2Cross(config = config, img_size= img_size, in_chans=input_channels)
...
if __name__=='__main__':
    ...
    model=Rolling_Unet_S(num_classes=1,input_channels=3,
                         deep_supervision=True,img_size=224)
    out=model(input)
