...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = SSA2D(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
...
class Rolling_Unet_S(nn.Module):
    def __init__(self, num_classes=1,input_channels=3,
                         deep_supervision=False,img_size=512,zero_head=False,
                          vis=False,):
    ...
    def load_from(self,):
        pretrained_path = "/mnt/Rolling-Unet-free-isic/vssmtiny_dp01_ckpt_epoch_292.pth"
...
if __name__=='__main__':
    ...
    model=Rolling_Unet_S(224,3).to('cuda')
    model.load_from()
    out=model(x)
